...
Image CreatePaddedImage(const Image& input, int padded_width, int padded_height) {
  Image padded;
  padded.width = padded_width;
  padded.height = padded_height;
  padded.channels = input.channels;
  padded.pixels.assign(static_cast<size_t>(padded_width) * padded_height * padded.channels, 0);

  for (int y = 0; y < input.height; ++y) {
    for (int x = 0; x < input.width; ++x) {
      for (int c = 0; c < input.channels; ++c) {
        const size_t src_idx = (static_cast<size_t>(y) * input.width + x) * input.channels + c;
        const size_t dst_idx = (static_cast<size_t>(y) * padded_width + x) * padded.channels + c;
...
size_t FlatPatchIndex(int px, int py, int channel, int patch_size, int channels) {
        ...
        for (int px = 0; px < patch_size; ++px) {
          for (int c = 0; c < image.channels; ++c) {
            const size_t src_idx =
                (static_cast<size_t>(y + py) * padded.width + (x + px)) * padded.channels + c;
            const size_t dst_idx = FlatPatchIndex(px, py, c, patch_size, image.channels);
...
Image ReconstructFromPatches(const PatchSet& patch_set, const PatchList& decoded_patches) {
    ...
    for (int py = 0; py < patch_set.patch_size; ++py) {
      for (int px = 0; px < patch_set.patch_size; ++px) {
        const size_t pixel_idx =
            static_cast<size_t>(start_y + py) * patch_set.padded_width + (start_x + px);
        counts[pixel_idx] += 1.0f;

        for (int c = 0; c < patch_set.channels; ++c) {
          const size_t src_idx =
              FlatPatchIndex(px, py, c, patch_set.patch_size, patch_set.channels);
  ...
  for (int y = 0; y < output.height; ++y) {
    for (int x = 0; x < output.width; ++x) {
      const size_t padded_pixel_idx = static_cast<size_t>(y) * patch_set.padded_width + x;
      const float count = std::max(counts[padded_pixel_idx], 1.0f);

      for (int c = 0; c < output.channels; ++c) {
        const size_t src_idx = padded_pixel_idx * output.channels + c;
        const size_t dst_idx = (static_cast<size_t>(y) * output.width + x) * output.channels + c;
        const float avg = accum[src_idx] / count;
        output.pixels[dst_idx] =
            static_cast<unsigned char>(std::clamp(avg, 0.0f, 1.0f) * 255.0f + 0.5f);
      }
    }
  }
